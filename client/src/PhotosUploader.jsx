...
const PhotosUploader = ({ addedPhotos, onChange }) => {
  ...
  async function addPhotoByLink(ev) {
    ev.preventDefault();
    if (!photoLink) {
      alert("Please provide the appropriate photo URL.");
      return;
    }
    ...
    try {
      ...
      onChange([...addedPhotos, data]); // Add new photo URL to state
  ...
  function uploadPhoto(ev) {
    ...
    axios
      ...
      .then((response) => {
        ...
        onChange([...addedPhotos, ...filenames]); // Add new photo filenames to state
      })
      .catch((error) => {
    ...
    <>
      ...
      <div className="flex gap-2">
        <input
          ...
          type="text"
          placeholder="Add an image through the link ... jpg"
          ...
          addedPhotos.map((link, index) => (
            <div className="h-32 flex relative" key={index}>
              ...
              <button
                ...
                className="absolute bottom-1 left-2 text-white bg-black bg-opacity-50 rounded-2xl py-2 px-3 cursor-pointer"
              >
                {link === addedPhotos[0] && (
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
